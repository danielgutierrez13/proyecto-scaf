...
@Component({
  selector: 'app-asignacion-crear',
  imports: [ReactiveFormsModule, RouterLink, MatButtonModule, MatFormFieldModule, MatSelectModule],
  templateUrl: './asignacion-crear.html',
  styleUrl: './asignacion-crear.scss',
})
export class AsignacionCrearComponent implements OnInit {
  ...
  protected guardar(): void {
    ...
    this.asignacionService.crear(this.obtenerPayload()).subscribe({
      next: () => this.router.navigate(['/layout/asignaciones']),
      error: () => {
        this.error = 'No se pudo crear la asignacion.';
